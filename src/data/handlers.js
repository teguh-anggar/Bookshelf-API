...
const addBookHandler = (request, h) => {
  ...
  const getAllBooksHandler = (request, h) => {
    ...
    const filteredBooks = books;
        ...
        if (name) {
            filteredBooks.filter((book) => book.name.toLowerCase().includes(name.toLowerCase()));
        };
        
        if (reading !== undefined) {
            const isReading = reading === "true";
            filteredBooks.filter((book) => book.reading === isReading);
        };
        if (finished !== undefined) {
            const isFinished = finished === "true";
            filteredBooks.filter((book) => book.finished === isFinished);
        };
